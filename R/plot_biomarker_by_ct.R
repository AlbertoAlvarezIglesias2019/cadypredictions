...
#' participants who developed CT ("Yes") and those who did not ("No").
#'
...
#'   containing the numeric biomarker values to be plotted on the y-axis.
...
#' @param title A character string for the main title of the plot. Defaults to \code{NULL} (no title).
#' @param subtitle A character string for the plot subtitle. Defaults to \code{NULL} (no subtitle).
#'
...
#'         lines are generated separately for the "Yes" and "No" groups.
#'   \item **Aesthetics**: "No" is mapped to **cyan/blue** (`#00BFC4`), and "Yes"
#'         is mapped to **red** (`#F8766D`). 
#'   \item **Non-Standard Evaluation (NSE)**: Columns are accessed via \code{.data[[column_name]]}.
#' }
#'
...
#' @examples
#' \dontrun{
#' # No title or subtitle (default behavior now)
#' plot_biomarker_by_ct(data = cardio_data, biomarker_name = "NTproBNP")
#'
#' # Manually specifying title and subtitle
#' plot_biomarker_by_ct(
#'   data = cardio_data,
#'   biomarker_name = "Troponin",
#'   title = "Troponin Levels",
#'   subtitle = "Comparing CT vs No-CT groups over 180 days"
#' )
#' }
#'
